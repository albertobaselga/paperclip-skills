...
### External-mode variant

For `external` bundles the four instruction files live on disk (typically inside a repo). Replace the `adapterConfig` block above with:
...
"adapterConfig": {
  ...
  "instructionsRootPath": "/absolute/path/.paperclip/agents/cto"
}
```

…and omit the `instructionsBundle` field entirely. Paperclip reads `AGENTS.md` (and any `HEARTBEAT.md` / `SOUL.md` / `TOOLS.md`) from `instructionsRootPath` at heartbeat time. The folder must exist on the host before the agent's first wake.
